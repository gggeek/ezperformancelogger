...
if (!empty($GLOBALS['run'])) {
  ...
  xhprof_render_image($xhprof_runs_impl, $GLOBALS['run'], $GLOBALS['type'],
                      $GLOBALS['threshold'], $GLOBALS['func'], $GLOBALS['source'], $GLOBALS['critical']);
} else {
  // diff report call graph image generation
  xhprof_render_diff_image($xhprof_runs_impl, $GLOBALS['run1'], $GLOBALS['run2'],
                           $GLOBALS['type'], $GLOBALS['threshold'], $GLOBALS['source']);
}
